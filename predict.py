...
class SAMPredictor:
    ...
    def __init__(self, model_path: str,
        ...
        assert self.model is not None, f"SAM模型加载失败，model_path: {model_path}"
...
def parse_args():
    ...
    parser.add_argument("--config", type=str, default="./config/sam_mobile.yaml",
                        choices=["./config/sam_1.yaml", "./config/sam_2.yaml", "./config/sam_mobile.yaml"],
                        help="配置文件路径，默认: config.yaml")
    parser.add_argument("--log_level", type=str, default="DEBUG",
                        choices=["DEBUG", "INFO", "WARNING", "ERROR"],
                        help="日志级别，可选: DEBUG/INFO/WARNING/ERROR，默认: INFO")
    parser.add_argument("--mode", type=str, default="img",
                        choices=["img", "video", "DynVideo"],
                        help="预处理模式，可选: 默认为空")
...
if __name__ == "__main__":
    ...
    if args.mode != "img" and args.config != "./config/sam_2.yaml":
        raise ValueError("sam_2.yaml 仅支持 DynVideo 模式")
    ...
    if args.mode in ["DynVideo", "video"]:
        input = config["input_mp4_path"]
    elif args.mode in ["img"]:
        input = config["input_jpg_path"]
    else:
        # error
        log_manager.error(f"❌ 未知模式 {args.mode}，请选择 img, video, DynVideo")
        exit(1)
    assert os.path.exists(input), f"输入文件 {input} 不存在"
    results = sam_predictor(input, bboxes=config["bboxes"], points=config["points"], labels=config["labels"])
    log_manager.info(f"✅ 推理完成，共处理 {len(results)} 个样本")

    if len(results) == 1:
        os.makedirs(config["output_path"], exist_ok=True)
        assert os.path.exists(config["output_path"]), f"输出路径 {config['output_path']} 不存在"
        results[0].save(f"{config['output_path']}/result.jpg")
        log_manager.info(f"✅ 输出文件保存在 {config['output_path']}/result.jpg")
    elif len(results) > 1:
